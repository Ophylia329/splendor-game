...
class EventHandler:
    ...
    def __init__(self, game_engine: GameEngine, renderer: Renderer):
        ...
        self.purchasable_cards = []  # 可购买的卡牌列表（购买模式下显示绿色边框）
        ...
        self.get_chinese_font = get_chinese_font
        self._update_fonts()

        # 设置初始欢迎提示
        self._set_hint("欢迎来到璀璨宝石Splendor，点击上方按钮开始游戏")
    ...
    def _update_fonts(self):
        """更新字体大小（根据当前缩放比例）"""
        s = self.renderer.scaler.scale_value
        self.font_small = self.get_chinese_font(int(s(config.FONT_SIZE_SMALL)))
        self.font_medium = self.get_chinese_font(int(s(config.FONT_SIZE_MEDIUM)))
        self.font_large = self.get_chinese_font(int(s(config.FONT_SIZE_LARGE)))

    def recreate_buttons(self):
        """重新创建按钮（窗口大小改变时调用）"""
        self.buttons = self._create_buttons()
        self._update_fonts()  # 同时更新字体大小
    ...
    def _handle_button_click(self, button_name: str):
        ...
        参数:
            button_name: 按钮名称
        """
        # 检查是否再次点击同一个动作按钮来取消操作
        if self.current_action is not None:
            current_action_button_map = {
                GameAction.TAKE_GEMS: 'take_gems',
                GameAction.RESERVE_CARD: 'reserve_card',
                GameAction.PURCHASE_CARD: 'purchase_card'
            }
            if current_action_button_map.get(self.current_action) == button_name:
                # 再次点击同一按钮，取消操作
                self._cancel_action()
                return
    ...
    def _start_take_gems_action(self):
        ...
        self.selected_gems = {}
        self._set_hint("请选择要拿取的宝石（3个不同颜色 或 2个相同颜色）| 再次点击按钮取消")
    ...
    def _start_reserve_card_action(self):
        ...
        self._set_hint("请选择要保留的卡牌（桌面卡牌或点击牌堆顶）| 再次点击按钮取消")
    ...
    def _start_purchase_card_action(self):
        ...
        # 检查所有可购买的卡牌
        self.purchasable_cards = []
        game_state = self.game_engine.game_state
        current_player = self.game_engine.get_current_player()

        # 检查桌面卡牌
        for level_cards in game_state.table_cards.values():
            for card in level_cards:
                if card:  # 确保卡牌不为None
                    valid, _ = self.game_engine.validator.validate_purchase_card(game_state, card)
                    if valid:
                        self.purchasable_cards.append(card)
        ...
        self._set_hint("请选择要购买的卡牌（桌面卡牌或自己的保留卡）| 再次点击按钮取消")
    ...
    def _clear_selection(self):
        ...
        self.purchasable_cards = []  # 清空可购买卡牌列表
    ...
    def _is_gem_selection_complete(self):
        """
        判断宝石选择是否完成
        ...
        num_colors = len(self.selected_gems)

        # 选了3个不同颜色
        if total == 3 and num_colors == 3:
            return True

        # 选了2个相同颜色
        if total == 2 and num_colors == 1:
            return True

        # 选了2个不同颜色（宝石种类不足时）
        if total == 2 and num_colors == 2:
            # 检查是否因为宝石种类不足
            # 计算公共宝石库中可用的宝石种类数量（不包括黄金）
            available_colors = 0
            for color in config.NORMAL_GEM_TYPES:
                if self.game_engine.game_state.gem_bank.get(color, 0) > 0:
                    available_colors += 1

            # 如果可用种类只有2种或更少，选2个不同颜色就算完成
            if available_colors <= 2:
                return True

        # 选了1个宝石（只剩1种时）
        if total == 1 and num_colors == 1:
            # 检查是否只剩1种宝石可选
            available_colors = 0
            for color in config.NORMAL_GEM_TYPES:
                if self.game_engine.game_state.gem_bank.get(color, 0) > 0:
    ...
    def _auto_confirm_action(self):
        """
        自动确认动作（弹出对话框并执行）
        """
        # 先显示确认对话框
        if not self._show_action_confirmation():
            # 用户取消，清空选择
            self._clear_selection()
            # 重新进入当前动作模式
    ...
    def _show_action_confirmation(self):
        """
        显示动作确认对话框（浮窗样式）
        ...
        button_y = dialog_y + dialog_height - s(60)
        ...
        while confirming:
            ...
            # 绘制标题
            title = self.font_large.render("确定结束此回合？", True, config.COLOR_WHITE)
            title_rect = title.get_rect(center=(dialog_x + dialog_width // 2, dialog_y + s(70)))
            screen.blit(title, title_rect)

            # 更新按钮悬停状态
            confirm_btn.is_hovered = confirm_btn.rect.collidepoint(mouse_pos)
            cancel_btn.is_hovered = cancel_btn.rect.collidepoint(mouse_pos)

            # 绘制按钮
            confirm_btn.draw(screen, self.font_medium)
            cancel_btn.draw(screen, self.font_medium)
        ...
        if self.current_action == GameAction.TAKE_GEMS:
            ...
            if gem_color and gem_color != 'gold':
                ...
                if gem_count > 0:
                    ...
                    # 检查是否选择完成，如果完成则自动弹出确认对话框
                    if self._is_gem_selection_complete():
                        self._auto_confirm_action()
                    return True
                else:
                    # 宝石数量为0，不处理点击
                    return False
            ...
            if current_count == 1:
                ...
                if len(self.selected_gems) == 1:  # 只选了这一种颜色
                    ...
                    if gem_count >= 4:
                        ...
                        self._set_hint(f"已选择2个相同颜色宝石 | 再次点击按钮取消")
                    else:
                        # 宝石数量 < 4，不能选2个，取消选择
                        del self.selected_gems[color]
                        self._set_hint(f"无法拿取两个相同宝石，剩余不足4个 | 请重新选择")
                else:
                    # 有其他颜色，取消这个颜色
                    del self.selected_gems[color]
                    # 更新提示
                    remaining = sum(self.selected_gems.values())
                    if remaining == 1:
                        self._set_hint("已选择1个宝石 | 再次点击按钮取消")
                    elif remaining == 2:
                        self._set_hint("已选择2个不同颜色宝石 | 再次点击按钮取消")
            else:  # current_count == 2
                # 选了2个，再点击就取消
                del self.selected_gems[color]
                self._set_hint("请选择要拿取的宝石 | 再次点击按钮取消")
        else:
            # 这个颜色还没选
            if total == 0:
                # 第一个宝石
                ...
                if len(self.selected_gems) == 1:
                    # 已经有2个相同颜色，不能再选
                    self._set_hint("已选择2个相同颜色宝石，无法再选择其他宝石 | 再次点击按钮取消")
                else:
                    # 已有2个不同颜色，可以再选1个
                    self.selected_gems[color] = 1
                    self._set_hint("已选择3个不同颜色宝石 | 再次点击按钮取消")
            else:
                # 已选择3个宝石，不能再选
                self._set_hint("已选择3个宝石，无法再选择更多 | 再次点击按钮取消")
    ...
    def draw_ui(self, screen):
        ...
        # 第一步：先更新所有按钮的启用状态
        for btn_name, btn in self.buttons.items():
            if btn_name in ['take_gems', 'reserve_card', 'purchase_card']:
                if self.current_action is None:
                    # 没有当前动作时，所有按钮启用
                    btn.set_enabled(True)
                else:
                    # 有当前动作时，只有对应的按钮保持启用（允许再次点击取消）
                    current_action_button_map = {
                        GameAction.TAKE_GEMS: 'take_gems',
                        GameAction.RESERVE_CARD: 'reserve_card',
                        GameAction.PURCHASE_CARD: 'purchase_card'
                    }
                    btn.set_enabled(current_action_button_map.get(self.current_action) == btn_name)
