...
class ActionExecutor:
    ...
    def check_and_award_nobles(self, game_state: GameState) -> List[Noble]:
        """
        检查哪些贵族可以拜访当前玩家（回合结束时调用）

        规则：每回合最多获得1个贵族，如果有多个满足条件，由玩家选择

        参数:
            game_state: 游戏状态

        返回:
            List[Noble]: 满足条件的贵族列表（让玩家选择）
        """
        player = game_state.get_current_player()
        qualified = self.validator.check_noble_visit(player, game_state.nobles)

        # 返回所有符合条件的贵族，让界面层处理选择
        return qualified
