...
COLOR_GREEN = (0, 255, 0)  # 绿色，用于可购买卡牌标记
...
CARD_WIDTH = 150  # 卡牌宽度（从140改为120）
CARD_HEIGHT = 225  # 卡牌高度（从200改为175）
CARD_MARGIN = 12  # 卡牌间距
...
def get_base_path():
    """获取程序基础路径，兼容PyInstaller打包环境"""
    if getattr(sys, 'frozen', False):
        # 如果是打包后的exe运行，使用_MEIPASS
        return sys._MEIPASS
    else:
        # 如果是源码运行，使用当前文件所在目录
...
DECKS_IMG_DIR = os.path.join(IMAGES_DIR, "decks")
BONUS_IMG_DIR = os.path.join(IMAGES_DIR, "bonus")
