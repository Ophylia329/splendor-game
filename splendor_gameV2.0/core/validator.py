...
class Validator:
    ...
    def validate_take_gems(game_state: GameState, gems: Dict[str, int]) -> Tuple[bool, str]:
        ...
        if total_take == 3:
            ...
            elif len(colors_taken) == 2:
                # 2个不同颜色（宝石种类不足时允许）
                for color in colors_taken:
                    if gems[color] != 1:
                        return False, "拿2个不同颜色时，每种颜色只能拿1个"

                    if gem_bank.get(color, 0) < 1:
                        return False, f"{color}宝石不足"
            else:
                return False, "拿2个宝石时只能是1种或2种颜色"

        elif total_take == 1:
            # 拿1个宝石（宝石种类不足时允许）
            if len(colors_taken) != 1:
                return False, "拿1个宝石时只能拿1种颜色"

            color = colors_taken[0]
            if gems[color] != 1:
                return False, "拿1个宝石时，该颜色只能拿1个"

            if gem_bank.get(color, 0) < 1:
                return False, f"{color}宝石不足"

        else:
            return False, f"只能拿1-3个宝石，当前拿取{total_take}个"

        # 规则4：拿宝石可以超过10个，但回合结束时需要弃置到10个
        # （不在这里检查，在回合结束时检查）
    ...
    def validate_reserve_card(game_state: GameState, card: Card, from_deck: bool = False) -> Tuple[bool, str]:
        ...
        # 规则3：保留卡可以拿黄金超过10个，但回合结束时需要弃置到10个
        # （不在这里检查，在回合结束时检查）
